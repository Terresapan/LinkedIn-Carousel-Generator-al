          ...
          if (responseText.length > 50) {
            try {
              ...
              if (transcript && transcript.length > 100) {
                ...
                console.log("Successfully extracted transcript, length:", cleanTranscript.length)
              ...
              if (responseText.length > 100) {
                ...
                if (cleanText.length > 100) {
                  ...
                  console.log("Successfully extracted transcript as plain text, length:", cleanText.length)
                  return cleanText
                }
              }
            }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!apiKey.startsWith("AIza")) {
      ...
      return new Response(JSON.stringify({ error: "Invalid Gemini API key format. Should start with 'AIza'" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      })
    }
    ...
    if (inputType === "youtube") {
      ...
      if (youtubeInputMethod === "manual" && (!manualTranscript || manualTranscript.length < 100)) {
        ...
        return new Response(JSON.stringify({ error: "Please provide a transcript with at least 100 characters" }), {
          status: 400,
          headers: { "Content-Type": "application/json" },
        })
      }
...
- Mention key insights from the carousel
- Use relevant hashtags (5-8 hashtags)
- Include a call-to-action to engage with the post
- Use emojis strategically for better engagement
- Write in a professional but conversational tone
- Mention that there's a downloadable carousel/PDF
